...
class M5RunConfig:
    ...
    dependency_active_pool_limit: int = 100
    dependency_activity_half_life: float = 40.0
...
def _generate_valid(dataset, config, backend, chapter, context, local, system, user):
    low, high = [
        int(item) for item in dataset.global_task.get(
            "allowed_chars_per_chapter", MECHANICAL_ACCEPTED_HAN_CHARS,
        )
    ]
    ...
    for attempt in range(config.retries + 1):
        ...
        feedback = (
            f"上一版正文汉字数为{count}，未通过机械校验；保持情节要求不变并"
            f"完整重写，正文仍严格控制在{requested_low}—{requested_high}个汉字。"
        )
...
def _dependency_config(config: M5RunConfig) -> PaperDependencyConfig:
    return PaperDependencyConfig(
        ...
        active_pool_limit=config.dependency_active_pool_limit,
        activity_half_life=config.dependency_activity_half_life,
    )
