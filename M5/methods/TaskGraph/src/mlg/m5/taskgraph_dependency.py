...
class PaperDependencyBuilder:
    ...
    method = "paper_vc_cdep_vr_model_judge_v6"
    ...
    def candidates(
        ...
        activity = {
            node.node_id: _activity_features(node, target, self.config)
            for node in historical
        }
        activity_ranked = sorted(
            historical,
            key=lambda node: (
                -activity[node.node_id]["score"],
                node.node_id,
            ),
        )
        activity_ranked = activity_ranked[:self.config.active_pool_limit]
        ref_active = {node.node_id for node in activity_ranked}
        embedding_scores = {
        ...
        candidate_ranking = [
            {
                "source_id": node_id,
                "fused_score": fused_scores[node_id],
                "semantic_score": semantic_scores.get(node_id, 0.0),
                "task_relevance": task_scores[node_id],
                "temporal_relevance": temporal_scores[node_id],
                "insertion_index": insertion[node_id],
            }
            for node_id in all_ranked
        ]
        ranked = all_ranked[:self.config.candidate_limit]
        details = []
        for source_id in ranked:
            channels = []
            if source_id in structural:
                channels.append("Vc")
            if source_id in ref_active:
                channels.append("Vr")
        ...
        audit = {
            "method": self.method,
            "fusion_formula": "Cfinal=rank_cap(Vc_union_Vr_union_Cdep)",
            "target_id": target_id,
            ...
            "Vr": [node.node_id for node in activity_ranked],
            "Cfinal": ranked,
            "candidate_union_count": len(final),
            "candidate_ranking": candidate_ranking,
            "candidate_details": details,
            "activity_pool": {
                "raw_count": len(historical),
                "selected_count": len(activity_ranked),
                "dormant_count": len(historical) - len(activity_ranked),
                "selected": [
                    {
                        "source_id": node.node_id,
                        "activity_score": round(activity[node.node_id]["score"], 6),
                        "age": activity[node.node_id]["age"],
                        "last_touch_chapter": activity[node.node_id]["last_touch_chapter"],
                        "ref_count": activity[node.node_id]["ref_count"],
                        "frequency": round(activity[node.node_id]["frequency"], 6),
                        "importance": activity[node.node_id]["importance"],
                        "unresolved": bool(activity[node.node_id]["unresolved"]),
                    }
                    for node in activity_ranked
                ],
            },
            "created": [],
            "config": {
                ...
                "active_pool_limit": self.config.active_pool_limit,
                "activity_half_life": self.config.activity_half_life,
                "activity_unresolved_weight": self.config.activity_unresolved_weight,
                "activity_frequency_weight": self.config.activity_frequency_weight,
                "activity_importance_weight": self.config.activity_importance_weight,
                "activity_frequency_saturation": self.config.activity_frequency_saturation,
            },
        }
...
def _activity_features(
    source: Node, target: Node, config: PaperDependencyConfig,
) -> dict[str, float | int]:
    """Compute transient retrieval activity; never mutate lifecycle truth."""
    target_chapter = int(target.metadata.get("chapter_id", 0) or target.turn_index)
    born_chapter = int(source.metadata.get("chapter_id", 0) or source.turn_index)
    activated_chapter = int(source.metadata.get("activated_at_chapter", 0) or 0)
    last_used = int(source.metadata.get("last_prompt_use_turn", 0) or 0)
    last_touch = max(0, born_chapter, activated_chapter, last_used)
    age = max(0, target_chapter - last_touch)
    ref_count = max(0, int(source.metadata.get("ref_count", 0) or 0))
    frequency = min(
        1.0,
        math.log2(1.0 + ref_count) / config.activity_frequency_saturation,
    )
    try:
        raw_importance = source.metadata.get("importance", 3)
        importance = 3.0 if raw_importance in (None, "") else float(raw_importance)
    except (TypeError, ValueError):
        importance = 3.0
    importance = max(0.0, min(5.0, importance))
    unresolved = int(
        source.metadata.get("state_status") == "active"
        or source.status == NodeStatus.ACTIVE
    )
    base = (
        config.activity_unresolved_weight * unresolved
        + config.activity_frequency_weight * frequency
        + config.activity_importance_weight * (importance / 5.0)
    )
    score = base * (2.0 ** (-age / config.activity_half_life))
    return {
        "score": score,
        "age": age,
...
def _compact_audit(audit: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "fusion_formula": audit.get("fusion_formula"),
        "candidate_union_count": int(audit.get("candidate_union_count", 0)),
        ...
        "activity_pool": {
            key: audit.get("activity_pool", {}).get(key, 0)
            for key in ("raw_count", "selected_count", "dormant_count")
        },
