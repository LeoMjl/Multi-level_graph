...
BASELINE_CONDITIONS = ("current_only", "recency_window", "running_summary")
class CollaborationBaselineRun:
    ...
    def prepare(self, chapter_id: int) -> Path:
        ...
        packet.update({
            ...
            "memory_audit": {
                **context.metadata,
                "selected_chapters": visible_chapters,
    ...
    def submit_chapter(
        ...
        if self.condition != "running_summary":
            self.memory.observe(self.dataset.release(chapter_id), text, None)  # type: ignore[arg-type]
            self._complete(chapter_id, [])
            return None
    ...
    def submit_summary(self, chapter_id: int, summary_path: Path,
        ...
        memory.prior_summary = memory.summary
        memory.prior_covered_through = memory.covered_through
        ...
        atomic_write_json(stored, {"summary": memory.summary})
        self.state["transaction"].update({
    ...
    def _complete(self, chapter_id: int, memory_calls: list[dict[str, Any]]) -> None:
        ...
        atomic_write_json(self.run_dir / "records" / f"chapter_{chapter_id:03d}.json", {
            ...
            "api_calls": [*tx["chapter_calls"], *memory_calls],
    ...
    def _write_summary_request(self, chapter_id: int, text: str) -> Path:
        if not isinstance(self.memory, RunningSummaryMemory):
            raise CollaborationProtocolError("Summary request requires running-summary memory")
        path = self.run_dir / "summary_requests" / f"chapter_{chapter_id:03d}.json"
        atomic_write_json(path, summary_request(self.memory, chapter_id, text))
        return path

    def _new_memory(self):
        return new_baseline_memory(self.condition, self.token_budget)
