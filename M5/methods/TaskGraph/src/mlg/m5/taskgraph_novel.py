...
class TaskGraphNovelMemory(MemoryStrategy):
    ...
    name = "taskgraph"
    state_schema = "m5-taskgraph-memory-v8"
    ...
    def apply_context_decisions(
        ...
        *,
        ...
        for node, edge in method_dependencies:
            ...
            source_chapter = int(
                node.metadata.get("chapter_id", 0)
                or node.metadata.get("activated_at_chapter", 0)
                or 0
            )
            if (
                source_chapter
    ...
    def _create_volume_skeleton(self) -> None:
        for volume_id in range(1, self.total_volumes + 1):
            ...
            node_id = self.graph.add_node(
                ...
                metadata={
                    "volume_id": volume_id, "expanded": False,
                    "released_chapters": 0, "importance": 3,
                    "ref_count": 0,
                },
            )
    ...
    def _release_chapter(self, prompt: ChapterPrompt) -> None:
        ...
        self.graph.nodes[volume_id].metadata.setdefault(
            "activated_at_chapter", prompt.chapter_id,
        )
    ...
    def _add_l4(self, chapter_id: str, prompt: ChapterPrompt, fact: dict[str, Any]) -> str:
        node_id = self.graph.add_node(
            ...
            metadata={
                ...
                "source_quote_valid": fact.get("source_quote_valid"),
                "source_quote_exact": fact.get("source_quote_exact"), "ref_count": 0,
            },
        )
