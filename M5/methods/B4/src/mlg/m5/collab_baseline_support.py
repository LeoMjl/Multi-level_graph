...
def baseline_protocol_fingerprint() -> str:
    ...
    paths = (
        ...
        Path(__file__).with_name("memory_extra.py"),
...
def new_baseline_memory(
    condition: str, token_budget: int, *, fake_embeddings: bool = False,
):
    ...
    if condition == "flat_vector_rag":
        embeddings = (
            FakeEmbeddingBackend()
            if fake_embeddings else LocalSentenceTransformerEmbeddingBackend()
        )
...
def hierarchical_summary_request(
    memory: HierarchicalSummaryMemory,
    prompt: ChapterPrompt,
    text: str,
) -> dict[str, Any]:
    prior_volume = memory.volume_summaries.get(prompt.volume_id, "")
    return {
        "schema": "m5-hierarchical-summary-request-v1",
        "chapter_id": prompt.chapter_id,
        "volume_id": prompt.volume_id,
        "system": (
            "你是分层小说连续性记忆器。只依据既有本卷摘要和刚完成的当前章正文更新记忆；"
            "不得读取或预测未来。严格输出一个JSON对象，且必须同时包含非空字符串字段"
            "chapter_summary与volume_summary。chapter_summary记录本章已发生事实；"
            "volume_summary把既有本卷摘要与本章事实合并为本卷累计摘要。两者都应保留人物"
            "知识边界、时间地点、物品位置/数量/损耗、承诺、未决线索和因果。"
        ),
        "user": (
            f"当前卷：第{prompt.volume_id}卷\n"
            f"既有本卷累计摘要：\n{prior_volume or '无'}\n\n"
            f"刚完成的第{prompt.chapter_id}章正文：\n{text}"
        ),
    }
