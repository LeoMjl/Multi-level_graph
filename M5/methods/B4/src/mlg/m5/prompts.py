...
def build_writer_prompts(
    ...
    *,
    ...
    if local_continuity is None:
        continuity = ""
        history_label = "此前已经建立、且可能与本章有关的事实"
        history = memory.text.strip() or "（没有可用的既有事实；不得自行补造此前发生的具体事件。）"
    else:
        previous = local_continuity.text.strip() or "（本章为开篇，没有上一章正文。）"
        continuity = (
            "本章开场承接材料（必须直接延续其时间、地点、人物状态与未完成动作）：\n"
            f"{previous}\n\n"
        )
        history_label = "其他当前有效事实"
...
def _chapter_contract(chapter: ChapterPrompt) -> str:
    ...
    return (
        ...
        f"必须包含：\n{include}\n"
        f"必须避免：\n{avoid}\n"
        f"目标长度：约{chapter.target_chars}个汉字，允许区间2000—3000个汉字。"
    )
