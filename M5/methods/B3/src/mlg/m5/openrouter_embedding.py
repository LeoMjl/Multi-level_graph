...
def _ordered_finite_vectors(
    data: Sequence[Any], *, expected_count: int,
) -> list[list[float]]:
    """Validate an embeddings response and restore request order by item.index."""
    if len(data) != expected_count:
        raise RuntimeError(
            "OpenRouter returned an incomplete embedding index set: "
    ...
    for item in data:
        ...
        if isinstance(index, bool) or not isinstance(index, int):
            raise RuntimeError("OpenRouter embedding item has a missing or invalid index")
        if not 0 <= index < expected_count:
            raise RuntimeError(f"OpenRouter embedding index is out of range: {index}")
        if index in by_index:
            raise RuntimeError(f"OpenRouter returned duplicate embedding index: {index}")
        ...
        try:
            ...
            raise RuntimeError(
                f"OpenRouter embedding vector at index {index} is invalid"
            ) from exc
        if not vector:
            raise RuntimeError(f"OpenRouter embedding vector at index {index} is empty")
        if any(not math.isfinite(value) for value in vector):
            raise RuntimeError(
                f"OpenRouter embedding vector at index {index} contains NaN or Inf"
            )
    ...
    if missing:
        raise RuntimeError(f"OpenRouter omitted embedding indices: {missing}")
