...
def parser() -> argparse.ArgumentParser:
    cli = argparse.ArgumentParser(description="Run the M1 LongMemEval-V2 experiment")
    ...
    run = commands.add_parser("run", help="Run the project methods")
    run.add_argument("--methods", default="ours_full,atomic_rag,bounded_context")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command == "fetch":
        for manifest in fetch_datasets("longmemeval_v2", "latest"):
            print(f"{manifest['dataset']}: {manifest['status']}")
        return
    if args.command == "prepare":
        print(prepare_experiment("m1", args.limit)[0])
        return
    result = run_suite(
        suite="main",
        experiments=["m1"],
